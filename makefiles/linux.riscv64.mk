...
rt_ha = rt/+linux/env.ha rt/+linux/initfini.ha rt/+linux/platform_abort.ha rt/+linux/platform_halt.ha rt/+linux/platformstart-libc.ha rt/+linux/segmalloc.ha rt/+linux/start.ha rt/+riscv64/arch_jmp.ha rt/abort.ha rt/ensure.ha rt/heap-libc.ha rt/jmp.ha rt/malloc.ha rt/memcpy.ha rt/memfunc_ptr.ha rt/memmove.ha rt/memset.ha rt/strcmp.ha rt/u64tos.ha
$(HARECACHE)/rt.ssa: $(rt_ha) $(HARECACHE)/sys.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
	@$(TDENV) $(HAREC) $(HARECFLAGS) -o $@ -t $(HARECACHE)/rt.td.tmp -N rt $(rt_ha)

rt_s = $(HARECACHE)/rt.s rt/+linux/restore+riscv64.s rt/+linux/start+riscv64-libc.s rt/+riscv64/longjmp.s rt/+riscv64/setjmp.s
$(HARECACHE)/rt.o: $(rt_s)
	@printf 'AS\t%s\n' "$@"
	@$(AS) $(ASFLAGS) -o $@ $(rt_s)
...
arch_ha = arch/+riscv64/cpuid.ha arch/fenv_defs.ha
$(HARECACHE)/arch.ssa: $(arch_ha)
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
	@$(TDENV) $(HAREC) $(HARECFLAGS) -o $@ -t $(HARECACHE)/arch.td.tmp -N arch $(arch_ha)

arch_s = $(HARECACHE)/arch.s arch/+riscv64/cpuid.s arch/+riscv64/fenv.s
...
$(HARECACHE)/math.ssa: $(math_ha) $(HARECACHE)/arch.td $(HARECACHE)/types.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
