# proto::io::
proto/io/file.gen.ha: $(PROTOS)/io/file.xml
	$(GENPROTO) < $<

protos: proto/io/file.gen.ha
$(HARECACHE)/io.ssa: proto/io/file.gen.ha

# proto::system::
proto/system/mman.gen.ha: $(PROTOS)/system/mman.xml
	$(GENPROTO) < $<

proto/system/process.gen.ha: $(PROTOS)/system/process.xml
	$(GENPROTO) < $<

protos: proto/system/process.gen.ha
...
clean-protos:
	...
	rm -f proto/io/*.gen*.s
	rm -f proto/system/*.gen.ha
	rm -f proto/system/*.gen*.s

.PHONY: protos clean-protos
# generated by cmd/genbootstrap
# DO NOT EDIT BY HAND. run 'make bootstrap' to update
TDENV = env HARE_TD_format::elf=$(HARECACHE)/format_elf.td HARE_TD_arch=$(HARECACHE)/arch.td HARE_TD_sys=$(HARECACHE)/sys.td HARE_TD_proto::system=$(HARECACHE)/proto_system.td HARE_TD_rt=$(HARECACHE)/rt.td HARE_TD_types=$(HARECACHE)/types.td HARE_TD_bytes=$(HARECACHE)/bytes.td HARE_TD_encoding::utf8=$(HARECACHE)/encoding_utf8.td HARE_TD_sort::cmp=$(HARECACHE)/sort_cmp.td HARE_TD_strings=$(HARECACHE)/strings.td HARE_TD_ascii=$(HARECACHE)/ascii.td HARE_TD_errors=$(HARECACHE)/errors.td HARE_TD_proto::io=$(HARECACHE)/proto_io.td HARE_TD_io=$(HARECACHE)/io.td HARE_TD_bufio=$(HARECACHE)/bufio.td HARE_TD_crypto::math=$(HARECACHE)/crypto_math.td HARE_TD_endian=$(HARECACHE)/endian.td HARE_TD_hash=$(HARECACHE)/hash.td HARE_TD_crypto::sha256=$(HARECACHE)/crypto_sha256.td HARE_TD_math=$(HARECACHE)/math.td HARE_TD_memio=$(HARECACHE)/memio.td HARE_TD_path=$(HARECACHE)/path.td HARE_TD_time=$(HARECACHE)/time.td HARE_TD_fs=$(HARECACHE)/fs.td HARE_TD_types::c=$(HARECACHE)/types_c.td HARE_TD_os=$(HARECACHE)/os.td HARE_TD_strconv=$(HARECACHE)/strconv.td HARE_TD_fmt=$(HARECACHE)/fmt.td HARE_TD_encoding::hex=$(HARECACHE)/encoding_hex.td HARE_TD_math::random=$(HARECACHE)/math_random.td HARE_TD_sort=$(HARECACHE)/sort.td HARE_TD_hare::lex=$(HARECACHE)/hare_lex.td HARE_TD_hare::ast=$(HARECACHE)/hare_ast.td HARE_TD_hare::parse=$(HARECACHE)/hare_parse.td HARE_TD_hare::unparse=$(HARECACHE)/hare_unparse.td HARE_TD_hare::module=$(HARECACHE)/hare_module.td HARE_TD_os::exec=$(HARECACHE)/os_exec.td HARE_TD_shlex=$(HARECACHE)/shlex.td HARE_TD_unix::tty=$(HARECACHE)/unix_tty.td HARE_TD_cmd::hare::build=$(HARECACHE)/cmd_hare_build.td HARE_TD_unix=$(HARECACHE)/unix.td HARE_TD_dirs=$(HARECACHE)/dirs.td HARE_TD_getopt=$(HARECACHE)/getopt.td HARE_TD_cmd::hare=$(HARECACHE)/cmd_hare.td
OBJS = $(HARECACHE)/format_elf.o $(HARECACHE)/arch.o $(HARECACHE)/sys.o $(HARECACHE)/proto_system.o $(HARECACHE)/rt.o $(HARECACHE)/types.o $(HARECACHE)/bytes.o $(HARECACHE)/encoding_utf8.o $(HARECACHE)/sort_cmp.o $(HARECACHE)/strings.o $(HARECACHE)/ascii.o $(HARECACHE)/errors.o $(HARECACHE)/proto_io.o $(HARECACHE)/io.o $(HARECACHE)/bufio.o $(HARECACHE)/crypto_math.o $(HARECACHE)/endian.o $(HARECACHE)/hash.o $(HARECACHE)/crypto_sha256.o $(HARECACHE)/math.o $(HARECACHE)/memio.o $(HARECACHE)/path.o $(HARECACHE)/time.o $(HARECACHE)/fs.o $(HARECACHE)/types_c.o $(HARECACHE)/os.o $(HARECACHE)/strconv.o $(HARECACHE)/fmt.o $(HARECACHE)/encoding_hex.o $(HARECACHE)/math_random.o $(HARECACHE)/sort.o $(HARECACHE)/hare_lex.o $(HARECACHE)/hare_ast.o $(HARECACHE)/hare_parse.o $(HARECACHE)/hare_unparse.o $(HARECACHE)/hare_module.o $(HARECACHE)/os_exec.o $(HARECACHE)/shlex.o $(HARECACHE)/unix_tty.o $(HARECACHE)/cmd_hare_build.o $(HARECACHE)/unix.o $(HARECACHE)/dirs.o $(HARECACHE)/getopt.o $(HARECACHE)/cmd_hare.o

format_elf_ha = format/elf/arch+x86_64.ha format/elf/types.ha
$(HARECACHE)/format_elf.ssa: $(format_elf_ha)
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
	@$(TDENV) $(HAREC) $(HARECFLAGS) -o $@ -t $(HARECACHE)/format_elf.td.tmp -N format::elf $(format_elf_ha)

arch_ha = arch/+x86_64/arch_jmp.ha arch/+x86_64/cpuid.ha arch/fenv_defs.ha arch/jmp.ha
$(HARECACHE)/arch.ssa: $(arch_ha)
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
	@$(TDENV) $(HAREC) $(HARECFLAGS) -o $@ -t $(HARECACHE)/arch.td.tmp -N arch $(arch_ha)

arch_s = $(HARECACHE)/arch.s arch/+x86_64/cpuid.s arch/+x86_64/fenv.s arch/+x86_64/longjmp.s arch/+x86_64/setjmp.s
$(HARECACHE)/arch.o: $(arch_s)
	@printf 'AS\t%s\n' "$@"
	@$(AS) $(ASFLAGS) -o $@ $(arch_s)

sys_ha = sys/+hermes/+x86_64.ha sys/+hermes/bootinfo.ha sys/+hermes/env.ha sys/+hermes/errno.ha sys/+hermes/funcs.ha sys/+hermes/server.ha sys/+hermes/syscalls.ha sys/+hermes/types.ha sys/u64tos.ha sys/unknown_errno.ha
$(HARECACHE)/sys.ssa: $(sys_ha) $(HARECACHE)/arch.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
	@$(TDENV) $(HAREC) $(HARECFLAGS) -o $@ -t $(HARECACHE)/sys.td.tmp -N sys $(sys_ha)

sys_s = $(HARECACHE)/sys.s sys/+hermes/+x86_64/call.s sys/+hermes/+x86_64/recv.s sys/+hermes/syscall+x86_64.s
$(HARECACHE)/sys.o: $(sys_s)
	@printf 'AS\t%s\n' "$@"
	@$(AS) $(ASFLAGS) -o $@ $(sys_s)

proto_system_ha = proto/system/process.gen.ha
$(HARECACHE)/proto_system.ssa: $(proto_system_ha) $(HARECACHE)/arch.td $(HARECACHE)/sys.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
	@$(TDENV) $(HAREC) $(HARECFLAGS) -o $@ -t $(HARECACHE)/proto_system.td.tmp -N proto::system $(proto_system_ha)

proto_system_s = $(HARECACHE)/proto_system.s proto/system/process.gen+x86_64.s
$(HARECACHE)/proto_system.o: $(proto_system_s)
	@printf 'AS\t%s\n' "$@"
	@$(AS) $(ASFLAGS) -o $@ $(proto_system_s)

rt_ha = rt/+hermes/env.ha rt/+hermes/platform_abort.ha rt/+hermes/platform_halt.ha rt/+hermes/platform_start.ha rt/+hermes/segmalloc.ha rt/+hermes/start.ha rt/abort.ha rt/ensure.ha rt/heap-libc.ha rt/initfini.ha rt/malloc.ha rt/memcpy.ha rt/memfunc_ptr.ha rt/memmove.ha rt/memset.ha rt/strcmp.ha rt/u64tos.ha
$(HARECACHE)/rt.ssa: $(rt_ha) $(HARECACHE)/format_elf.td $(HARECACHE)/proto_system.td $(HARECACHE)/sys.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
	@$(TDENV) $(HAREC) $(HARECFLAGS) -o $@ -t $(HARECACHE)/rt.td.tmp -N rt $(rt_ha)

rt_s = $(HARECACHE)/rt.s rt/+hermes/start+x86_64.s
$(HARECACHE)/rt.o: $(rt_s)
	@printf 'AS\t%s\n' "$@"
	@$(AS) $(ASFLAGS) -o $@ $(rt_s)
...
proto_io_ha = proto/io/file.gen.ha
$(HARECACHE)/proto_io.ssa: $(proto_io_ha) $(HARECACHE)/arch.td $(HARECACHE)/sys.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
	@$(TDENV) $(HAREC) $(HARECFLAGS) -o $@ -t $(HARECACHE)/proto_io.td.tmp -N proto::io $(proto_io_ha)

proto_io_s = $(HARECACHE)/proto_io.s proto/io/file.gen+x86_64.s
$(HARECACHE)/proto_io.o: $(proto_io_s)
	@printf 'AS\t%s\n' "$@"
	@$(AS) $(ASFLAGS) -o $@ $(proto_io_s)

io_ha = io/+hermes/mmap.ha io/+hermes/platform_file.ha io/arch+x86_64.ha io/copy.ha io/drain.ha io/empty.ha io/file.ha io/handle.ha io/limit.ha io/stream.ha io/tee.ha io/types.ha io/unread.ha io/util.ha io/zero.ha
$(HARECACHE)/io.ssa: $(io_ha) $(HARECACHE)/bytes.td $(HARECACHE)/errors.td $(HARECACHE)/proto_io.td $(HARECACHE)/rt.td $(HARECACHE)/sys.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
...
path_ha = path/+hermes.ha path/buffer.ha path/error.ha path/ext_stack.ha path/iter.ha path/posix.ha path/prefix.ha path/stack.ha
...
time_ha = time/+hermes/functions.ha time/arithm.ha time/duration.ha time/instant.ha
$(HARECACHE)/time.ssa: $(time_ha) $(HARECACHE)/errors.td $(HARECACHE)/format_elf.td $(HARECACHE)/math.td $(HARECACHE)/rt.td $(HARECACHE)/sys.td $(HARECACHE)/types.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
	@$(TDENV) $(HAREC) $(HARECFLAGS) -o $@ -t $(HARECACHE)/time.td.tmp -N time $(time_ha)

time_s = $(HARECACHE)/time.s time/+hermes/tsc+x86_64.s
...
os_ha = os/+hermes/dirfdfs.ha os/+hermes/exit.ha os/+hermes/klog.ha os/+hermes/platform_environ.ha os/+hermes/status.ha os/+hermes/stdfd.ha os/bufio.ha os/environ.ha os/os.ha
$(HARECACHE)/os.ssa: $(os_ha) $(HARECACHE)/bufio.td $(HARECACHE)/errors.td $(HARECACHE)/fs.td $(HARECACHE)/io.td $(HARECACHE)/rt.td $(HARECACHE)/strings.td $(HARECACHE)/sys.td $(HARECACHE)/time.td $(HARECACHE)/types_c.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
...
os_exec_ha = os/exec/cmd.ha os/exec/types.ha
$(HARECACHE)/os_exec.ssa: $(os_exec_ha) $(HARECACHE)/errors.td $(HARECACHE)/fs.td $(HARECACHE)/io.td $(HARECACHE)/os.td $(HARECACHE)/path.td $(HARECACHE)/strings.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
