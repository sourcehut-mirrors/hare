...
sys_ha = sys/+haiku/+x86_64.ha sys/+haiku/errno.ha sys/+haiku/libc.ha sys/+haiku/signal.ha sys/+haiku/syscalls.ha sys/+haiku/types.ha sys/u64tos.ha sys/unknown_errno.ha
$(HARECACHE)/sys.ssa: $(sys_ha)
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
	@$(TDENV) $(HAREC) $(HARECFLAGS) -o $@ -t $(HARECACHE)/sys.td.tmp -N sys $(sys_ha)

rt_ha = rt/+haiku/env.ha rt/+haiku/platform_abort.ha rt/+haiku/start.ha rt/abort.ha rt/ensure.ha rt/initfini.ha rt/malloc+libc.ha rt/memcpy.ha rt/memfunc_ptr.ha rt/memmove.ha rt/memset.ha rt/strcmp.ha rt/u64tos.ha
...
os_ha = os/+haiku/dirfdfs.ha os/+haiku/exit.ha os/+haiku/fcntl.ha os/+haiku/fs.ha os/+haiku/platform_environ.ha os/+haiku/shm.ha os/+haiku/status.ha os/+haiku/stdfd.ha os/bufio.ha os/environ.ha os/os.ha
...
os_exec_ha = os/exec/+haiku/exec.ha os/exec/+haiku/platform_cmd.ha os/exec/+haiku/process.ha os/exec/cmd.ha os/exec/types.ha
$(HARECACHE)/os_exec.ssa: $(os_exec_ha) $(HARECACHE)/errors.td $(HARECACHE)/fs.td $(HARECACHE)/io.td $(HARECACHE)/os.td $(HARECACHE)/path.td $(HARECACHE)/strings.td $(HARECACHE)/sys.td $(HARECACHE)/types_c.td $(HARECACHE)/unix.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
