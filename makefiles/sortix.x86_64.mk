...
rt_ha = rt/+sortix/+x86_64.ha rt/+sortix/env.ha rt/+sortix/errno.ha rt/+sortix/initfini.ha rt/+sortix/ioctl.ha rt/+sortix/platform_abort.ha rt/+sortix/readdir.ha rt/+sortix/signals.ha rt/+sortix/socket.ha rt/+sortix/start.ha rt/+sortix/syscalls.ha rt/+sortix/types.ha rt/+x86_64/arch_jmp.ha rt/+x86_64/cpuid.ha rt/abort.ha rt/ensure.ha rt/fenv_defs.ha rt/jmp.ha rt/malloc+libc.ha rt/memcpy.ha rt/memfunc_ptr.ha rt/memmove.ha rt/memset.ha rt/strcmp.ha rt/u64tos.ha rt/unknown_errno.ha
$(HARECACHE)/rt.ssa: $(rt_ha)
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
	@$(TDENV) $(HAREC) $(HARECFLAGS) -o $@ -t $(HARECACHE)/rt.td.tmp -N rt $(rt_ha)

rt_s = $(HARECACHE)/rt.s rt/+sortix/start.s rt/+x86_64/cpuid.s rt/+x86_64/fenv.s rt/+x86_64/longjmp.s rt/+x86_64/setjmp.s
$(HARECACHE)/rt.o: $(rt_s)
	@printf 'AS\t%s\n' "$@"
	@$(AS) $(ASFLAGS) -o $@ $(rt_s)
...
path_ha = path/+sortix.ha path/buffer.ha path/error.ha path/ext_stack.ha path/iter.ha path/posix.ha path/prefix.ha path/stack.ha
...
time_ha = time/+sortix/functions.ha time/arithm.ha time/duration.ha time/instant.ha
...
os_ha = os/+sortix/dirfdfs.ha os/+sortix/exit.ha os/+sortix/fcntl.ha os/+sortix/fs.ha os/+sortix/platform_environ.ha os/+sortix/status.ha os/+sortix/stdfd.ha os/bufio.ha os/environ.ha os/os.ha
...
unix_ha = unix/+sortix/creds.ha unix/+sortix/pipe.ha unix/+sortix/rlimit.ha unix/+sortix/umask.ha
...
unix_signal_ha = unix/signal/+sortix.ha unix/signal/types.ha
...
os_exec_ha = os/exec/+sortix/exec.ha os/exec/+sortix/platform_cmd.ha os/exec/+sortix/process.ha os/exec/cmd.ha os/exec/types.ha
...
unix_tty_ha = unix/tty/+sortix/isatty.ha unix/tty/+sortix/open.ha unix/tty/+sortix/pgid.ha unix/tty/+sortix/pty.ha unix/tty/+sortix/termios.ha unix/tty/+sortix/winsize.ha unix/tty/types.ha
...
$(HARECACHE)/cmd_hare.ssa: $(cmd_hare_ha) $(HARECACHE)/ascii.td $(HARECACHE)/bufio.td $(HARECACHE)/cmd_hare_build.td $(HARECACHE)/dirs.td $(HARECACHE)/errors.td $(HARECACHE)/fmt.td $(HARECACHE)/fs.td $(HARECACHE)/getopt.td $(HARECACHE)/hare_ast.td $(HARECACHE)/hare_lex.td $(HARECACHE)/hare_module.td $(HARECACHE)/hare_parse.td $(HARECACHE)/io.td $(HARECACHE)/memio.td $(HARECACHE)/os.td $(HARECACHE)/os_exec.td $(HARECACHE)/path.td $(HARECACHE)/rt.td $(HARECACHE)/shlex.td $(HARECACHE)/sort.td $(HARECACHE)/sort_cmp.td $(HARECACHE)/strconv.td $(HARECACHE)/strings.td $(HARECACHE)/time.td $(HARECACHE)/unix_tty.td
	@mkdir -p -- "$(HARECACHE)"
	@printf 'HAREC\t%s\n' "$@"
